...
`TimeField` manages a wall-clock `Time` value:

```tsx
import { TimeField, type Time } from "@alydev/datepicker";

const initial: Time = { hour: 9, minute: 30, second: 0, millisecond: 0 };

<TimeField defaultValue={initial} hourCycle={12} withSeconds variant="field" />;
```
